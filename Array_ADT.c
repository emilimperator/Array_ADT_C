...
int main(void){
    ...
    while(!stop){
        ...
        if(a<=arr.size){
            ...
            printf("The number of elements that are requested should be smaller or equal to the size of an array.\n");
        }
    }
...
void Append(struct Array *arr, int element){
    if(arr->length<arr->size){
        ...
        printf("The length of an array cannot be bigger than the size of an array.\n");
}
...
int Sum_Recursive(struct Array arr, int size){
    if(size<1) return 0;
    else return Sum_Recursive(arr, size-1)+arr.A[size-1];
}
